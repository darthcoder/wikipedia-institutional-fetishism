...
The `data/` directory contains only:
- `88_categories.json` — The official 88-category map from Wikipedia (1,462 unique articles). Generated by parent project's `scripts/fetch_category_map.py`. Static reference data, rarely changes.

### From Parent Project

The actual corpus is always sourced from the parent project (`ner-paper`):
- **Location**: `../../data/wwi_extended.jsonl` (relative to this directory)
- **Schema**: `{pageid, title, wikitext}` — see parent project's fetch pipeline
- **Content**: 3,230 WWI articles, raw wikitext (not cleaned)
- **Size**: ~15 MB
- **Format**: JSONL (one JSON object per line)

Ghost hunt also accepts a `text` field as an alternative to `wikitext` for preprocessed versions.
