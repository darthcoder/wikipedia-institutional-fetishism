...
```bash
uv run python ignore/wikipedia-institutional-fetishism/analysis/ghost_hunt.py
```

The corpus (`data/wwi_extended.jsonl`) is not stored here — it lives in the parent project's `data/` directory and is referenced by relative path from the analysis scripts. The `--corpus` flag overrides the default path on all scripts.
...
| File | Description |
|---|---|
| `ghost_mentions.jsonl` | Every (term, article) pair with positions and 150-char context windows |
...
| `corpus_index.pkl` | Pickled inverted index (auto-generated, ~50–100 MB) |
...
The analysis runs against `data/wwi_extended.jsonl` from the parent project:
- 3,230 articles (as of April 2026)
- Raw wikitext (not cleaned) — ghost hunt and collision work on wikitext directly
- corpus_index.py tokenizes on the fly during build (whitespace-split tokens)

The `data/` directory here contains only the category map (`88_categories.json`) from `scripts/fetch_category_map.py` (88 categories, 1,462 unique articles). The actual corpus is always sourced from the parent project.

**Corpus schema**: `{pageid, title, wikitext}` (see parent project's fetch pipeline). Ghost hunt also accepts `text` field as alternative to `wikitext`.
